...
```bash
npm run start:dev && node bin/any-factory produce-products --specs="a:10;b:20;c:30" --concurrency=1 --with-server
```
...
上面除了讨论了如何构建可观察和可动态修改的命令行程序以外，还应用了领域驱动设计的方法指导实现。当然，由于领域驱动设计是个相当大的主题，本文中无法详细描述。敬请期待接下来的文章吧。
